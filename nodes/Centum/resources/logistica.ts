...
const crearRemitoCompra: ResourceHandler = async (context) => {
	...
	void consumerApiPublicId;
					...
					try {
						const dataCliente = await helperFns.apiRequest<any>(`${centumUrl}/Clientes/${idCliente}`, { method: "GET", headers });
...
const crearRemitoVenta: ResourceHandler = async (context) => {
	...
	void consumerApiPublicId;
					...
					const turnoDeEntrega = executeFunctions.getNodeParameter("turnoEntrega", 0);
					const fechaDocumento = executeFunctions.getNodeParameter("documentDate", 0) as string;
					const fechaDocumentoFormateada = fechaDocumento.replace(/\..+/, "");
					const fechaEmbarque = executeFunctions.getNodeParameter("shipmentDate", 0) as string;
					...
					const fechaEntrega = executeFunctions.getNodeParameter("deliveryDate", 0) as string;
					const fechaEntregaFormateada = fechaEntrega.replace(/\..+/, "");
					const idVendedor = executeFunctions.getNodeParameter("idVendedor", 0);
					const idCliente = executeFunctions.getNodeParameter("clienteId", 0);
					const rawArticles = executeFunctions.getNodeParameter("articlesCollection", 0);
